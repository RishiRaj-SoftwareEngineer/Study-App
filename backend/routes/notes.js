...
router.delete('/admin/:id', VerifyJwtMiddleware, authMiddleware, requireRole('admin'), async (req, res) => {
    try {
        ...
        // Remove the file from Cloudinary (if present)
        if (note.filePublicId) {
            try {
        ...
        // Remove note from owner's list and delete it
        await User.findByIdAndUpdate(note.uploadedBy, { $pull: { notes: note._id } });
        await note.deleteOne();

        // Decrement the owner's note counter
        await UserState.findOneAndUpdate({ user: note.uploadedBy }, { $inc: { totalNotes: -1 } });

        res.status(200).json({ message: 'Note deleted successfully', noteId: id });
    } catch (error) {
